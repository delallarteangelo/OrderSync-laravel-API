...
class PosSaleService
{
    ...
    public function finalize(
        ...
        return DatabaseDialect::withTransactionLock(
            "pos:{$business->getKey()}:{$idempotencyKey}",
            function () use ($business, $cashier, $role, $request, $idempotencyKey, $fingerprint, $paymentMethod, $normalizedLines, $tenderedMinor, $paymentReference): array {
                ...
                $settings = $business->settings()->firstOrNew();
                $taxRateBasisPoints = $settings->tax_rate_basis_points;
                $taxableMinor = $subtotalMinor - $discountTotalMinor;
                $taxTotalMinor = intdiv($taxableMinor, 10000) * $taxRateBasisPoints
                    + intdiv(($taxableMinor % 10000) * $taxRateBasisPoints + 5000, 10000);
