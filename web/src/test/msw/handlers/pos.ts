...
export const posHandlers = [
  ...
  http.post("/api/v1/pos/sales", async ({ request }) => {
    ...
    const subtotal = body.lines.reduce((a, l) => a + l.unitPrice * l.quantity, 0);
    const discountTotal = body.lines.reduce((a, l) => a + (l.lineDiscount ?? 0), 0);
    const taxRate = db.settings.taxRate ?? 0;
    const taxTotal = Math.round((subtotal - discountTotal) * (taxRate / 100) * 100) / 100;
    const grandTotal = subtotal - discountTotal + taxTotal;
    const change =
      body.paymentMethod === "CASH" && body.tendered != null
        ? Math.max(0, body.tendered - grandTotal)
        : undefined;
    ...
    for (const l of body.lines) {
      ...
      if (pi !== -1) {
        db.products[pi] = {
          ...db.products[pi],
          stockOnHand: db.products[pi].stockOnHand - l.quantity,
        };
        db.movements = [
          {
            id: randomId("mv"),
